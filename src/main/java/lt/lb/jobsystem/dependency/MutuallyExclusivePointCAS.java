 ...
 * job can be in multiple exclusive points. Using compare and swap (CAS)
 * technique.
 *
 * Doesn't check every job that can access this mutually exclusive point,
 * instead uses "dibs" system alongside with CAS.
 *
 * Works better for mutually exclusive point with many contesting threads
 * (jobs).
 *
 * @author laim0nas100
 */
public class MutuallyExclusivePointCAS implements Dependency {
    ...
    public void addSharingJob(final Job job) {
        ...
        jobs.put(id, job);
        job.addListener(SystemJobEventName.ON_FAILED_TO_START, (j, c, d) -> {
